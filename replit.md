...
**Username login and multi-role authorization:** account login uses the unique lowercase `users.username` field; it is not email validation. `orders.customerEmail` remains separate customer contact data. `user_roles` is the sole authorization source; `users` has no role column and each account may have one or more canonical role rows. Before publishing this post-cleanup code to production, verify that every production account already has at least one `user_roles` row. Publish without selecting **overwrite data** and confirm the schema diff renames the legacy account identifier from `users.email` to `users.username` and drops only the legacy `users.role` column; do not proceed if it includes other user-data changes. Full model and write invariants → `docs/architecture-sources-of-truth.md` §Auth.
