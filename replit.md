...
**Temporary multi-role production rollout:** this release is stage one of a two-Publish change. It deliberately keeps the nullable `users.role` column alongside `user_roles`; authorization uses `user_roles` when rows exist and falls back to `users.role` only for a legacy-only account. Publish stage one without selecting **overwrite data**, sign in as an admin, open **Admin → Users**, run **Initialize roles**, and confirm the panel reports that every account has stored role assignments. Stage two is a later code change and Publish: only after that green verification, remove the legacy column, fallback, initialization endpoint, and migration panel.
