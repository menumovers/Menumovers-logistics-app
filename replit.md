...
**Multi-role production cleanup:** `user_roles` is the sole authorization source. Before publishing the cleanup, verify every production account has canonical role rows. Publish without selecting **overwrite data** and confirm the schema diff drops only `users.role`; do not proceed if it includes other user data changes.
