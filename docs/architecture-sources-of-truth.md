...
- **Why**: the source sends both representations independently. Storing both as writable made them disagree permanently on the first coordinator correction (`todo-bugs.md` B5). One writable copy removes the failure rather than synchronising it. See D12.
