...
- **What it does**: Produces the i18n descriptor for the human-readable countdown next to a pickup time — except for two statuses where it isn't a pickup-time countdown at all (see "Do not" below).
- **Logic**: Let `m = minutesUntil(iso)`. If `|m| >= 60`, return the absolute clock time as a literal. If `m === 0`, return `common.now`. If `m < 0`, return `pickup.late` with `minutes = |m|`. Else return `pickup.in` with `minutes = m`.
- **Callers**: `components/pickup-countdown.tsx`, coordinator and rider list cards.
- **Do not**: manually concatenate "min te laat" — it's localized and the cutoff is centralized. Do not call this for `order.status === "en_route_to_customer"` or `"delivered"` — `PickupCountdown` branches around it for both (see next entry).

### Pickup urgency, and the two statuses that aren't a countdown at all
- **Location**: `artifacts/bestellenbij/src/lib/format.ts` → `urgencyFor`; class map in `artifacts/bestellenbij/src/lib/status.ts` → `URGENCY_CLASS`; branch point in `artifacts/bestellenbij/src/components/pickup-countdown.tsx`.
- **What it does**: Maps "minutes until pickup" to a five-state urgency (`neutral`, `warn`, `danger`, `late`, `lateAtRestaurant`) that drives the colored priority indicator — but only while the order is still pending pickup. Once picked up, a late pickup is no longer the rider's problem to see colored red, and once the order is en route or delivered, "minutes until pickup" isn't a meaningful number at all.
- **Logic**:
  - `urgencyFor(iso, now, status)`: `m < 0` → `late`, unless `status === "arrived_at_restaurant"`, in which case `lateAtRestaurant` (the rider is there; a late pickup past that point is the restaurant's delay, not something to blame the rider for — hence a distinct orange rather than red). `m < 5` → `danger`; `m < 20` → `warn`; otherwise `neutral`. If `status` is `picked_up`, `en_route_to_customer`, or `delivered`, always `neutral` regardless of `m` — the countdown is done mattering once the food has actually been collected.
  - `PickupCountdown` (the component, not the formula) branches *ahead* of `urgencyFor`/`pickupCountdownLabel` entirely for two statuses that need different information, not just a different color: `en_route_to_customer` shows a live "N min underway" count via `minutesSince(order.enRouteToCustomerAt)` (see next entry); `delivered` shows the fixed delivered clock time via `formatTime(order.updatedAt)` (safe once terminal — see "`updatedAt` as a terminal timestamp" in Data Access, below). `picked_up` alone still renders through the normal (now-neutral) countdown path.
- **Callers**: `pickup-countdown.tsx`, every list/detail card across all three apps (restaurant, rider, coordinator) since they share the one component.
- **Do not**: invent ad-hoc thresholds like "≤ 10 min". Do not add a new "is this status special" branch anywhere but `PickupCountdown` itself — `urgencyFor`/`pickupCountdownLabel` should stay pure functions of `(iso, now, status)`.

### En-route timer
- **Location**: column `orders.en_route_to_customer_at` (`lib/db/src/schema/orders.ts`); set in `artifacts/api-server/src/routes/orders.ts` (`POST /orders/:id/status` when `toStatus === "en_route_to_customer"`, and `POST /orders/:id/resume` when resuming into that status); read via `minutesSince` in `artifacts/bestellenbij/src/lib/format.ts`.
- **What it does**: Anchors "how long has the rider been underway" independently of `updatedAt`, which the order can still pick up further writes after (a coordinator's note, a pickup-time edit) while the rider is en route — using `updatedAt` here would make the elapsed count lie. Moves forward again if the order is reported back into `en_route_to_customer` after leaving it (a correction, or a resume out of postponement), so it always reflects the start of the *current* leg, not the first one.
- **Do not**: derive "time underway" from `updatedAt` or from a status-log scan on a list endpoint — the column exists specifically so this is a free field on the row, not a query.

### Pickup-time sort ordering
- **Location**: `artifacts/bestellenbij/src/lib/format.ts` → `comparePickupTime`
- **What it does**: The one ordering used by every order-list view (restaurant, rider, coordinator overviews) — ascending by each order's effective pickup time (`effectivePickup(o).iso`, so it already accounts for the override→restaurant→rider→original priority chain). Applied once to the freshly-fetched list, before any per-section filtering — `Array.prototype.filter` preserves relative order, so every board column, section, and bundle group on all three pages inherits it for free.
- **Callers**: `pages/coordinator.tsx`, `pages/rider.tsx`, `pages/restaurant.tsx`.
- **Do not**: sort a fetched order list a different way, or leave it in API response order (creation order) — before this existed, none of the three overview pages applied any ordering at all, so cards appeared in whatever order the database happened to return them.
...
- **Logic**: Status is a report, not a gate — any transition is accepted except where an invariant forbids it. (1) `pending` and `rider_assigned` are never settable here; both are coupled to `riderId` and written together by `POST /orders/:id/assign`. (2) `delivered` and `failed` are terminal; only `delivered → failed` leaves, which is intentional. (3) No same-state transitions — the route's atomic guard depends on the status moving. (4) `rider_accepted` is settable here only when `from === "rider_assigned"` — otherwise, like `pending`/`rider_assigned`, it's reachable solely and atomically via `POST /orders/:id/assign` (rider self-claim, or trip assignment). Skipping ahead and correcting backwards are both accepted everywhere else; the status log records the jump that actually happened. See `docs/workflow-decisions.md` D1 and D16.
...
### Rider reassignment and unassignment
- **Location**: `artifacts/api-server/src/routes/orders.ts` → `POST /orders/:id/reassign`
- **What it does**: The only path that can change or clear the rider on an order that's already past `pending` — `POST /orders/:id/assign` is atomic *from* pending only and refuses anything else. Passing a `riderId` swaps in that rider; omitting it (or passing `null`) unassigns back to `pending`.
- **Logic**: Reachable at any non-terminal, non-held stage — deliberately no "hasn't left for the restaurant yet" cutoff on unassign, matching reassign-to-a-different-rider, which never had one either (see D18). Swapping to a new rider while the order is still pre-flight (`rider_assigned`/`rider_accepted`) treats it as a fresh assignment (`rider_accepted`, skipping the accept step — same convention as `PATCH /trips/:id`'s in-motion rider swap and trip creation's pending→rider_accepted skip). Once en route or later, only the rider changes; the reported status is preserved rather than rewound. Unassigning always sets `status: "pending"` regardless of how far along the order is — coordinators are trusted to have a reason (D18).
- **Do not**: reintroduce a status-dependent cutoff on the unassign path — that was tried and deliberately removed, see D18. Do not call this for a still-`pending` order (no rider to change) — use `POST /orders/:id/assign`.
...
- **Callers**: trip creation's bundleable check, trip creation's pending→rider_accepted transition, `PATCH /trips/:id`'s in-flight detection and unassign/reassign branches, `POST /trips/:id/dissolve`'s pre-flight-reverts-to-pending check, `POST /trips/:id/orders`'s bundleable check (shares `attachOrdersToTrip` with creation), `DELETE /trips/:id/orders/:orderId`'s detach branch (shares `detachOrderFromTrip` with dissolve). Six call sites before this constant existed, each spelling out `{pending, driver_assigned}` (now three statuses, not two) by hand.
- **Do not**: reintroduce a hand-spelled `["pending", "rider_assigned", "rider_accepted"]` (or worse, the old two-status list) at a new call site — import this constant. Do not confuse it with `RIDER_COUPLED` in `state-machine.ts`, which answers a different question (which statuses can never be a bare status report) and does not include `rider_accepted`.
...
- **Concurrency safety**: every multi-statement trip mutation in `routes/trips.ts` (`POST /trips`, `PATCH /trips/:id`, `PUT /trips/:id/stops`, `POST /trips/:id/dissolve`, `POST /trips/:id/orders`, `DELETE /trips/:id/orders/:orderId`) runs inside `db.transaction(async (tx) => …)` and begins by locking the trip row with `SELECT ... FOR UPDATE`. Terminal-state checks (`status in ('dissolved','completed')`) and order-status reads happen inside the same transaction so a concurrent rider advance, dissolve, or rename cannot leave partial stops or partial order linkage.
...
- **Membership editing**: bundling is not create-time only. `POST /trips/:id/orders` (`{ orderIds }`) adds more bundleable orders to an existing trip — same eligibility as creation (unattached, pre-flight) — and appends their stops after the trip's existing ones (reorder afterward via `PUT /trips/:id/stops` if needed). `DELETE /trips/:id/orders/:orderId` removes one — pre-flight reverts it to `pending`/unassigned, in-flight just drops the trip link, mirroring what dissolve already did per order. The attach/detach logic itself lives in two shared functions, `attachOrdersToTrip` and `detachOrderFromTrip` in `routes/trips.ts`, used by creation+add and dissolve+remove respectively — not reimplemented at each call site.
- **Completion**: `completeTripIfDone` (`artifacts/api-server/src/lib/trip-completion.ts`) marks a trip `completed` once every order still on it — or there are none left — is `delivered`/`failed`. Called after a status report lands an order on delivered/failed (`routes/orders.ts`) and after `DELETE /trips/:id/orders/:orderId` (an order vacuously satisfies "all done" when there are none left, so emptying a trip via removal completes it the same way finishing every delivery does). It's a derived check against the trip's current orders on every call, not a cached/stored flag, so it stays correct across concurrent order updates without its own locking. See D19 — `completed` existed in the `trip_status` enum from day one but nothing had ever set it before this.
- **Callers**: coordinator UI (`pages/coordinator-trip-builder.tsx`, `pages/coordinator-trip.tsx` — including its `AddOrdersCard` and per-order remove button, `pages/coordinator.tsx` `TripsSection`), rider UI (`pages/rider.tsx` trips section, `pages/rider-trip.tsx`, `pages/rider-order.tsx` trip banner + postpone/resume), restaurant UI (`pages/restaurant.tsx` `BundleSection` — grouping/summary only; `pages/restaurant-order.tsx` renders the bundle's individual orders).
- **Do not**: render trips by `tripId` UUID in user-facing copy — always use `tripNumber`. Do not compute the bundle pickup time on the client; rely on `bundlePickupTime` from the API. Do not write a trip mutation outside a `db.transaction` — the row-lock invariant is what makes the concurrent-edit guarantees hold. Do not bypass the `TRIP_IN_MOTION` guard at the call site by issuing `force: true` without surfacing the warning to the user. Do not reimplement the attach/detach per-order logic at a new call site — call `attachOrdersToTrip`/`detachOrderFromTrip`. Do not add a separate "is this trip done" flag — `completeTripIfDone` re-derives it from the orders every time on purpose.
...
- **Write rule**: `POST /users` creates the account and its initial role rows in one transaction. `PATCH /users/:id` locks the parent user row before reading its current role rows and applying the requested add/remove diff. This serializes concurrent role edits so a later transaction always derives its diff from the committed set.
- **Callers**: `routes/auth.ts` (login), `requireAuth`, every `requireRole`-protected API route, `routes/users.ts` (admin account management), and frontend guards through `CurrentUser.roles`.
- **Do not**: add a role field or role cache to `users`. Do not treat a JWT's `roles` claim as the current server authority. Do not reduce a multi-role account to a single "primary" role for access checks; `primaryRoleLabel` exists only for audit-log attribution. When changing an existing account's role set, do not calculate or apply the role diff outside the locked `PATCH /users/:id` transaction.

### Auth middlewares
- **Location**: `artifacts/api-server/src/lib/auth.ts` → `requireAuth`, `requireRole`, `requireInboundCredential`
- **What it does**: `requireAuth` validates the bearer token (or `auth_token` cookie), checks revocation, loads the active account and its current `user_roles` rows, and resolves `riderId` if `rider` is among them. `requireRole(...allowed)` gates on whether any of the caller's roles is in `allowed`; it does not infer a hierarchy. `requireInboundCredential` matches the `x-inbound-secret` header against a hashed per-source secret in `api_credentials` and sets `req.inboundSource` from the matched row — the caller's source comes from which credential matched, never from the request body. This replaced an earlier single-shared-secret mechanism (`requireInboundSecret` / `INBOUND_SHARED_SECRET`) in a direct swap, not an addition alongside it — there's no fallback to the old secret once this code is deployed.
...
- **Do not**: serialize an order ad hoc in a route. Add a new serializer here if you need a new shape. **Exception**: `GET /orders/history` deliberately does not use this — see "Order history and PII redaction" below for why.

### `updatedAt` as a terminal timestamp
- **Location**: column `orders.updated_at` (`lib/db/src/schema/orders.ts`, `.$onUpdate(() => new Date())` — bumped by Drizzle on every `.update()` call, regardless of which columns changed).
- **What it does**: Bumped on *any* change to the order row, not only status — so it is **not** a status-change timestamp for a non-terminal order. It becomes trustworthy as "when did this reach its current state" only once a status is terminal (`delivered`/`failed`) and therefore unlikely to be touched again.
- **Callers**: `pickup-countdown.tsx`'s delivered-clock-time branch (see "Pickup urgency" above); `GET /orders/history`'s sort/filter/redaction anchor (see next entry) — both reuse a delivered/failed order's `updatedAt` as "when this happened" instead of adding a dedicated `deliveredAt` column.
- **Do not**: read `updatedAt` as a status-change timestamp for a non-terminal order — it lies the moment anything else about the order is edited. If you need a true "entered this status at" timestamp for a *non*-terminal status, that's what `enRouteToCustomerAt` is for (see above) — add a dedicated column for the specific status you need, don't reach for `updatedAt`.

### Order history and PII redaction
- **Location**: `artifacts/api-server/src/routes/orders.ts` → `GET /orders/history`; schema `OrderHistoryItem` / `OrderHistoryPage` in `openapi.yaml`.
- **What it does**: Paginated (10/page), delivered/failed-only order history, scoped to the caller exactly like `GET /orders` (reuses `orderScopeWhere` unmodified — the terminal-status filter already makes its rider "open pending pool" clause a no-op, so no new scoping logic was needed). Filterable by an inclusive `updatedAt` date range.
- **Redaction**: once more than 24 hours have passed since `updatedAt` (see the entry above), `customerName`/`customerPhone` are redacted to `null` and `deliveryAddress` is reduced to postal code + city, computed server-side before the row is ever serialized — the PII never reaches the browser for an old order, it isn't merely hidden by the client. Every other field (items, total, status, timestamps) stays visible indefinitely. See D20.
- **Why a standalone response schema**: `OrderHistoryItem` does not extend `OrderListItem` via `allOf`, on purpose. OpenAPI/JSON-Schema `allOf` is an intersection, not an override — re-declaring `customerName` as nullable in a second `allOf` branch does not loosen the base (non-nullable) type, so Orval would have generated `customerName: string`, not `string | null`, and the redaction would have been unrepresentable in the generated type. `OrderHistoryItem` is instead a deliberately narrow, independent object with just the fields a history row needs.
- **Do not**: extend `OrderListItem`/`Order` via `allOf` to loosen a field's nullability for one endpoint's response — see above. Do not redact on the client — compute and drop the fields server-side, before serialization. Do not derive the redaction anchor from anything but `updatedAt` on a terminal order (see the entry above).
...
### Receipt rendering and auto-print
- **Location**: Receipt presentation in `artifacts/bestellenbij/src/pages/order-receipt.tsx`; restaurant entry point in `pages/restaurant-order.tsx`; readiness/scheduling rule in `artifacts/bestellenbij/src/lib/receipt-autoprint.ts`.
- **What it does**: Renders the printable kitchen receipt at `/orders/:id/receipt`. Normal navigation exposes a manual Print button. The restaurant order-detail receipt action adds the one-shot `autoprint=true` query so the browser print dialog opens automatically after the complete receipt is rendered.
- **Logic**: Auto-print requires the order query and the matching restaurant record (`restaurant.id === order.restaurantId`) to be ready. The helper schedules the callback on the next animation frame; the page removes the query with `history.replaceState` before calling `window.print()`.
- **Callers**: `pages/restaurant-order.tsx` for the automatic restaurant flow; `pages/order-receipt.tsx` for both automatic and manual printing. `src/lib/receipt-autoprint.test.ts` covers the order-first/restaurant-later race.
- **Do not**: call `window.print()` as soon as only the order query resolves. Do not print a partial receipt with a missing restaurant name or address. Do not remove the manual Print button or make coordinator receipt navigation auto-print unless that workflow is intentionally changed. Do not add API or schema fields for this browser-only intent.
...
| Orders | `routes/orders.ts` | `/api/orders/*`, `/api/inbound/orders` | POST inbound (per-source hashed credential), GET list/detail (admin-only archived view), GET history (paginated, PII-redacted after 24h — registered ahead of GET `/:id` so `history` is never captured as an `:id`), archive/restore/permanent delete, assign/reassign/status/resume/pickup-time, restaurant accept/ready, hold/release/resolve-restaurant, item hide/add, contact, rider-notification |
...
| Trips | `routes/trips.ts` | `/api/trips/*` | GET list (role-scoped: riders see own, restaurant_staff see none), GET by id, POST create (admin/coordinator), PATCH rename/reassign (admin/coordinator; returns 409 `TRIP_IN_MOTION` unless `force: true`), PUT replace stops (admin/coordinator), POST/DELETE `:id/orders[/:orderId]` add/remove orders (admin/coordinator), POST dissolve (admin/coordinator/own-rider). All multi-statement mutations run inside `db.transaction(...)` with `SELECT ... FOR UPDATE`. |
...
- **2026-08-19** — Restaurant page rebuilt on the rider's list/detail pattern. `pages/restaurant.tsx` is now overview-only — compact cards (`pickupCountdownLabel` pill, pickup time, order # + customer name) that link to a new `pages/restaurant-order.tsx`. Every action (acknowledge, ready-for-pickup, kitchen note, item overrides, receipt link, suggest pickup time) moved to the detail page, mirroring `pages/rider.tsx` → `pages/rider-order.tsx`. Bundle grouping (`BundleSection`) stays on the overview as a summary header only — no actions — with each bundled order still a card linking out individually. New component `components/acceptance-status-badge.tsx` renders a restaurant-only "New order" (`#A0CFD7`) / "Accepted" (`#E2F0D9`) pill from `restaurantAcceptedAt`, independent of the order-lifecycle `StatusBadge`. Side effect: the old overview issued one `useGetOrder` per card just to read `itemOverrides`; the detail page's single fetch already has them, so that N+1 pattern is gone.
- **2026-08-19** — Pickup countdown threshold widened from ±30 to ±60 minutes (`pickupCountdownLabel` in `lib/format.ts`, SSOT entry above updated). Shared by every screen that renders the pill (rider and restaurant, list and detail), so the change is uniform rather than restaurant-specific. `pages/restaurant-order.tsx` also gained: item `options`/`notes` rendered one per row under each item (same format as `order-receipt.tsx`, extended to a third renderer — see the D15 entry above), the kitchen note moved below the items list, and the acknowledge/suggest-pickup-time/ready-for-pickup block moved below the kitchen note, in that vertical order.
- **2026-08-19** — Closed a restaurant-visibility gap in D2. `orderScopeWhere`'s `restaurant_staff` clause never applied `notHeld()` — only the rider-discovery branch did — so a restaurant could see and open an order a coordinator had put on hold, contradicting D2's "not restaurants" visibility rule. The restaurant clause now ANDs `notHeld()` alongside the `restaurantId` match, so held orders drop out of both `GET /orders` and `GET /orders/:id` (via `loadOrderForAuthOr404`) for restaurant_staff, same as they already did for rider discovery.
- **2026-08-20** — Restaurant receipt action now requests immediate browser printing via `?autoprint=true` instead of requiring a second click on the receipt page. `order-receipt.tsx` keeps the manual fallback, waits for both order and restaurant data before printing, schedules after the receipt commits, and removes the one-shot query first. Added the focused `receipt-autoprint.test.ts` regression test; no API or schema change.
- **2026-08-19** — `driver_assigned` renamed `rider_assigned`; two new statuses added (D16). `rider_accepted` sits between `rider_assigned` and `en_route_to_restaurant` — a coordinator/admin assignment lands on `rider_assigned` and waits for the rider to accept, while a rider self-claim or a trip assignment lands directly on `rider_accepted` (claiming already implies acceptance; trips get no accept UI of their own). `arrived_at_restaurant` sits between `en_route_to_restaurant` and `picked_up`. New state-machine rule 4 (SSOT entry above) keeps `rider_accepted` from being reachable by report-skipping from anywhere but `rider_assigned`. `Order` gained a serialized `updatedAt` (the column already existed; it just wasn't exposed) — needed by the coordinator board's "delivered in the last hour" window below. New SSOT entry "Pre-flight order statuses (trips)" for the constant that replaced six hand-spelled `{pending, driver_assigned}` checks in `trips.ts`.
- **2026-08-19** — Coordinator overview rebuilt as a three-column board, same list/detail split as rider and restaurant. `pages/coordinator.tsx` is now compact cards linking to the existing `pages/coordinator-order.tsx`; nothing changed on the detail page itself, which already rendered `allowedTransitions` generically. Columns: Your orders (`riderId` matches the coordinator's own, when they have one) + Open orders; Active orders (`rider_assigned`/`rider_accepted`/`en_route_to_restaurant`/`arrived_at_restaurant`); In delivery (`picked_up`/`en_route_to_customer`) + Completed orders (`delivered` in the last hour, via the new `updatedAt` field). New "Needs attention" section pulls `postponed` and recently-`failed` orders out of the board entirely, the same way the existing Held section pulls held orders out — neither status fits a column by itself. Card layout: pickup countdown + restaurant name with a 2-character restaurant-acceptance pill (`AcceptanceStatusBadge` gained a `compact` variant); pickup time with an asap/scheduled icon (new `PickupTimeTypeIcon`, extracted from `RequestedTimeLabel` in `delivery-expectation.tsx` so rider/coordinator cards use the same glyph); assigned rider + the order's full status text. Rider overview cards gained the same pickup-type icon.
