...
- **Callers**: trip creation's bundleable check, trip creation's pending→rider_accepted transition, `PATCH /trips/:id`'s in-flight detection and unassign/reassign branches, `POST /trips/:id/dissolve`'s pre-flight-reverts-to-pending check. Six call sites before this constant existed, each spelling out `{pending, driver_assigned}` (now three statuses, not two) by hand.
