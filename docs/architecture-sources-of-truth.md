...
- **Location**: `artifacts/api-server/src/lib/pickup-time.ts` → `resolveOriginalPickupTime`, `resolveAsapLeadMinutes`, `leadTimeMinutes`
...
- **Logic**:
  - **ASAP** → `sourceCreatedAt + (inTheMoment ?? MAX(restaurantMinPickupTime, deliveryTeamMinPickupTime) ?? default)`. Entirely source data. `MinPickupTime` is a *minimum lead time*, and for an ASAP order the earliest possible pickup is checkout plus that lead time; the larger of the two figures binds because whichever party needs longer decides.
  - **`later_today` / `other_day`** (and any unrecognised type) → `requestedDeliveryTime − default`. Our own estimate, stated as ours: the payload carries no trustworthy travel figure.
  - **Lead time** → `requestedDeliveryTime − sourceCreatedAt`. Pure observation, logged at ingestion. Not a gate.
- **Settings**: `pickupEstimateDefaultMinutes` (baseline, 20) and `pickupEstimateInTheMomentMinutes` (today's conditions, ASAP only, cleared at 03:00 Europe/Amsterdam by the janitor via `lib/daily-reset.ts`).
- **Callers**: `artifacts/api-server/src/routes/orders.ts` (`POST /inbound/orders`, insert branch only).
- **Do not**: read the clock. Nothing in this path may use `new Date()` — an ASAP pickup time anchors to `sourceCreatedAt`, so an ingestion delay cannot push it later. That was `todo-bugs.md`'s original defect and the point of D13.
- **Do not**: derive a travel time from `*MinDeliveryTime`. It is checkout-to-doorstep — the whole journey including prep — and a rough figure a coordinator sets and rarely revisits. `MinDeliveryTime − MinPickupTime` is arithmetically valid and epistemically junk: the result looks derived while inheriting the reliability of the worse input. We have no trustworthy travel figure; say so rather than manufacturing one.
- **Do not**: feed `sourceRestaurantReadyTime` into the formula. The source calculates it for ASAP orders only, so depending on it means two code paths where one will do. It is retained as a cross-check.
- **Do not**: apply the in-the-moment value to a scheduled order. It describes conditions right now; an order for next Tuesday is not happening now.
- **Do not**: clamp the result, reject a late order, or otherwise repair infeasibility. Bestellenbij owns what the customer is offered. Surface the lead time and let a coordinator ask.
- **Do not**: read `*MinPrepTime`. Legacy at the source.
...
- **2026-08-15** — Temporal mapping rebuilt (D13), superseding D4's formula and closing the "audit every computed time" question. Nothing in the ingestion path reads the clock: ASAP counts forward from `sourceCreatedAt`, scheduled works back from `requestedDeliveryTime`. `*MinPickupTime` is finally read (as a minimum lead time, larger of the two binds); `*MinDeliveryTime` and `sourceRestaurantReadyTime` become audit-only; `*MinPrepTime` is legacy. New settings `pickupEstimateDefaultMinutes` (20) and `pickupEstimateInTheMomentMinutes` (ASAP only, cleared 03:00 Europe/Amsterdam), replacing `pickupTravelOverrideMinutes`. New `lib/daily-reset.ts` handles the DST-aware boundary; the janitor clears the value rather than expiring it on read, so stored state stays true. Lead time (`requestedDeliveryTime − sourceCreatedAt`) is logged as an observation — feasibility is upstream's, not ours.
