...
For each item: where it came from, whether it is already in `FUTURE_WORK.md` (the roadmap register), whether it is a **deferred feature** (we expect to build it eventually) or a **conscious boundary** (an architectural choice not to build this here), and any implementation notes captured at the time.
...
- **In FUTURE_WORK.md**: No
- **Kind**: Deferred feature
- **Notes**: The local email/password auth system (bcryptjs + JWT HS256, JTI revocation) was intentionally designed to be replaceable. The `auth.ts` helpers (`hashPassword`, `verifyPassword`, `signToken`, `verifyToken`) are centralized so an OAuth/SAML/OIDC layer can be swapped in without touching route handlers. When this becomes necessary, the most natural seam is replacing `signToken`/`verifyToken` and the `POST /api/auth/login` handler while keeping `requireAuth` and `requireRole` intact. `FUTURE_WORK.md` does not cover this — it is an infrastructure task, not a product feature.
...
- **In FUTURE_WORK.md**: Yes — item 1 (Quality & Reliability)
...
- **Source**: Tasks #2, #3
- **In FUTURE_WORK.md**: Yes — item 14 (Architecture)
...
- **In FUTURE_WORK.md**: Yes — item 5 (Quality & Reliability)
- **Kind**: Deferred feature
...
- **Source**: Task #2 (Backend Implementation)
- **In FUTURE_WORK.md**: Yes — item 6 (Operations & Admin)
- **Kind**: Deferred feature
...
- **Source**: Task #2 (Backend Implementation)
- **In FUTURE_WORK.md**: Yes — item 7 (Operations & Admin)
- **Kind**: Deferred feature
...
- **Source**: Task #2 (Backend Implementation)
- **In FUTURE_WORK.md**: Yes — item 8 (Operations & Admin)
- **Kind**: Deferred feature
...
- **Source**: Task #3 (Frontend PWA)
- **In FUTURE_WORK.md**: Yes — item 9 (Operations & Admin)
- **Kind**: Deferred feature
...
- **In FUTURE_WORK.md**: Yes — item 10 (Operations & Admin)
- **Kind**: Deferred feature
...
- **Source**: Task #3 (Frontend PWA)
- **In FUTURE_WORK.md**: Yes — item 11 (UX Improvements)
- **Kind**: Deferred feature
...
- **Source**: Tasks #3, #5
- **In FUTURE_WORK.md**: Yes — item 12 (UX Improvements)
- **Kind**: Deferred feature
...
- **Source**: Task #3 (Frontend PWA)
- **In FUTURE_WORK.md**: Yes — item 13 (UX Improvements)
- **Kind**: Deferred feature
...
These items were declared out of scope specifically in Task #5 (Trip Bundling) and are not currently in `FUTURE_WORK.md`. They range from natural next steps to deliberate boundaries.
...
| Item | In FUTURE_WORK.md | In docs/todo.md |
|---|---|---|
