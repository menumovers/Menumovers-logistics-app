...
import("@workspace/db").then(async ({ db, restaurantsTable, usersTable, userRolesTable, ridersTable, apiCredentialsTable }) => {
  ...
  // A postponed unassigned order resumes to pending so it returns to dispatch.
  const postponePending = await j("POST",`/orders/${id}/status`,{ toStatus:"postponed" }, { authorization: `Bearer ${token}`});
  console.log("postpone pending:", postponePending.status, postponePending.body.status);
  if (postponePending.status !== 200 || postponePending.body.status !== "postponed") {
    throw new Error("Expected an unassigned order to postpone");
  }
  const resumePending = await j("POST",`/orders/${id}/resume`,null, { authorization: `Bearer ${token}`});
  console.log("resume pending (expect pending):", resumePending.status, resumePending.body.status);
  if (resumePending.status !== 200 || resumePending.body.status !== "pending") {
    throw new Error("Expected a postponed unassigned order to resume to pending");
  }
  // Assign rider
  ...
  const bad = await j("POST",`/orders/${id}/status`,{ toStatus:"pending" }, { authorization: `Bearer ${token}`});
  ...
  // An in-motion postponement resumes at the exact prior delivery step.
  const postponeInMotion = await j("POST",`/orders/${id}/status`,{ toStatus:"postponed" }, { authorization: `Bearer ${token}`});
  console.log("postpone in motion:", postponeInMotion.status, postponeInMotion.body.status);
  if (postponeInMotion.status !== 200 || postponeInMotion.body.status !== "postponed") {
    throw new Error("Expected an in-motion order to postpone");
  }
  const resumeInMotion = await j("POST",`/orders/${id}/resume`,null, { authorization: `Bearer ${token}`});
  console.log("resume in motion (expect en_route_to_restaurant):", resumeInMotion.status, resumeInMotion.body.status);
  if (resumeInMotion.status !== 200 || resumeInMotion.body.status !== "en_route_to_restaurant") {
    throw new Error("Expected an in-motion order to resume to its prior status");
  }
